...
class MADDPG(object):
    ...
    def __init__(self, env, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound, memory_capacity=10000, max_steps=10000, target_tau=0.01, target_update_steps=500, reward_gamma=0.99, reward_scale=1.0, done_penalty=None, training_strategy='centralized', actor_output_act=torch.tanh, actor_lr=0.01, critic_lr=0.01, optimizer_type='adam', entropy_reg=0.01, max_grad_norm=None, batch_size=100, episodes_before_train=100, epsilon_start=0.9, epsilon_end=0.05, epsilon_decay=100, use_cuda=False):
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            ...
            target_q = self.reward_scale * rewards_var[:, agent_id, :] + self.reward_gamma * next_q * (1.0 - dones_var)
    ...
    def exploration_action(self, state):
        ...
        epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * np.exp(-1.0 * self.n_episodes / self.epsilon_decay)
