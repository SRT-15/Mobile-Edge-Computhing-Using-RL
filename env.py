...
class MecBCEnv(object):
    ...
    def __init__(self, n_agents, S_DDL=1, S_EPSILON=0.86, W_BANDWIDTH=20, S_one_power=20, S_one_gamma=0.6, mode='normal'):
        self.state_size = 10
        self.action_size = 6
    ...
    def reset(self):
        ...
        State_ = []
        State_ = [[self.S_channel[n], self.S_power[n], self.S_gain[n], self.S_size[n], self.S_cycle[n], self.S_resolu[n], self.S_ddl[n], self.S_res[n], self.S_com[n], self.S_epsilon[n]] for n in range(self.n_agents)]
    ...
    def step(self, action):
        ...
        DataRate = self.W_BANDWIDTH * np.log(1 + S_power * S_gain / (NOISE_VARIANCE + total_power - x_n * S_power * S_gain)) / np.log(2)
